...
from utils.db_func import take_verif_tasks_to_complet, set_rework_task


@dp.message_handler(commands="rework")
async def rework_task_func(message: types.Message, state: FSMContext):
    tasks_titles = await take_verif_tasks_to_complet(message.from_user.id)
    if tasks_titles:
        async with state.proxy() as data:
            data['ref10'] = tasks_titles
        await message.answer('Введите навзание задачи для отправки на доработку или "отмена" для возврата в главное '
                             'меню')
...
async def set_task_deadline(message: types.Message, state: FSMContext):
    if message.text.lower() != 'отмена':
        ...
        if message.text in tasks_titles:
            await set_rework_task(message.text)
